...
This directory now includes a minimal local preprocessing entrypoint for the flat parquet schema.
...
Print a schema summary without writing outputs:

```bash
python prepare_dataset.py --summary-only
```

Validate the dataset and write time-ordered train/valid parquet splits:

```bash
python prepare_dataset.py --output-dir outputs/prepared --valid-ratio 0.2
```

Implementation notes:

- The loader validates the required ID and label columns: `user_id`, `item_id`, `label_type`, `label_time`, `timestamp`.
- Feature groups are inferred from column prefixes so the same code path can be reused when the full dataset is released.
- The default split sorts by `label_time`, then reserves the most recent 20% of rows for validation while keeping `label_type` as the supervised target.
